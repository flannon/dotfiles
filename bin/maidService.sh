...
DATE=`/bin/date`
LOG=/Users/stan/log/"maidService.log"
...
/bin/rm -rf /Users/stan/.Trash/* 2>&1 1>>$LOG
...
if [ "$TODAY" -eq "5" ]; then
    ...
    /usr/sbin/diskutil secureErase freespace 3 /dev/disk0s2  2>&1 1>>$LOG
    echo "$DATE - Finished weekly cleanup." >> $LOG

else
    #echo "This is a regular day of the week."
    echo "$DATE - Starting 7-pass daily cleanup." >> $LOG
    /usr/sbin/diskutil secureErase freespace 2 /dev/disk0s2 2>&1 1>>$LOG
